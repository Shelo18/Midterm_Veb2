...
*/
  ...
  function getSubjectsStatistics(students) {
   let Average = (students.reduce((Sum, stutendt) => sum + students.grades,0))/3;
    return students.map((student) => ({
        name:student.firstNamename + " " + student.lastName,
        
    }));
  };
  
  function getHighAchievers(students, gradeThreshold) {
  return  students.filter((student) => (students.reduce((Sum, stutendt) => sum + students.grades,0))/3 > gradeThreshold).map((student) => ({
        name:student.firstNamename + " " + student.lastName,
    }));
  };
  ...
  function getMultiAchivers(students, threshold) {
    let n=0;
    if(students.grades.math > threshold || students.grades.history > threshold || students.grades.physicsh > threshold) {
        n++;
    }
   ...
   return students.map((student) => ({
    name:student.firstNamename + " " + student.lastName,
    expertSubjects: n
   })); //ვიცი რომ არასწორია ბატონო გიორგი, ვერ მოვემზადე სათანადოდ
  }; 
